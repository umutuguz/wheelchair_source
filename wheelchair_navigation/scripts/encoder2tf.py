...
#############################################################################
    ...
    #############################################################################
        ...
        pi = 3.141592654 
        wheel_diameter = 0.390
        tics_per_rev = 46080  #1440 * 32 
        # calibration_constant = 1.0
        self.tics_correction_left = 0.980
        self.tics_correction_right = 0.991
        # self.tics_correction_left = 1.005
        # self.tics_correction_right = 1

        wheel_circum =  pi * wheel_diameter
        tics_per_meter = (tics_per_rev / wheel_circum) 

        # wheel circum = 2* 3.14 * 0.225 = 1.225220113
        # One rotation encoder ticks : 1440 ticks
        # For 1 meter: 1440 * ( 1 / 1.413) = 1019.108280255 ticks

        self.rate = rospy.get_param('~rate',100.0)  # the rate at which to publish the transform
        self.ticks_meter = float(rospy.get_param('ticks_meter', tics_per_meter))  # The number of wheel encoder ticks per meter of travel
        self.base_width = float(rospy.get_param('~base_width', 0.59)) # The wheel base width in meters
	...
	self.roll = 0.01
        ...
        # subscriptions
        rospy.Subscriber("/motor_monitor", MotorMonitor, self.lwheelCallback)
        rospy.Subscriber("/motor_monitor", MotorMonitor, self.rwheelCallback)
    ...
    #############################################################################
        ...
        if now > self.t_next:
            ...
            if self.enc_left == None:
                ...
                d_left = (self.left - self.enc_left) / (self.ticks_meter * self.tics_correction_left)
                d_right = (self.right - self.enc_right) / (self.ticks_meter * self.tics_correction_right)
