...
namespace local_planner
{
    ...
    bool LocalPlanner::computeVelocityCommands(geometry_msgs::Twist &cmd_vel)
    {
        ...
        linearVel = (coefVel * ((0.4 * log((3.5 * (dmin_temp - 0.15)) + 0.0)) / (exp(0.883)) + (exp(1.57) / 6.5))) + 0.01;
        ...
        else if (dmin < 1.0)
        {
            ...
            cmd_vel.linear.x = linearVelocity*exp(-(1.0 - dmin));
            // cmd_vel.linear.x = 0.0;
            cmd_vel.linear.y = 0.0;
            cmd_vel.linear.z = 0.0;
    ...
    double LocalPlanner::LLCallback()
    {
        ...
        {
            if (currRange[i + 1] > currRange[i] + 1.5)
            {
                gap_number = gap_number + 1;
                gap_starting_points.push_back(i);
            }
            if (currRange[i] > currRange[i + 1] + 1.5)
            {
        ...
        for (int i = 0; i < midpoint_memory.size(); i++)
        {
            ...
            for (int j = 0; j < midpoint_memory.size() ; j++)
            {
                ...
                {
                    if(sqrt(pow(midpoint_memory[i][0]-midpoint_memory[j][0],2) + pow(midpoint_memory[i][1]-midpoint_memory[j][1],2)) < 1.1)
                    {
        ...
        for (int i=0; i < gaps_in_memory.size(); i++)
        {
            ...
            {
                gaps_in_memory[i][2] = gaps_in_memory[i][2] * 5.2;
