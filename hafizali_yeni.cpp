...
namespace local_planner
{
                                  ...
                                  costmap_2d::Costmap2DROS *costmapROS)
    {
        if (!initialized_)
        {
            ...
            initialized_ = true;
            myfile.open("/home/otonom/wheelchair_ws/src/log/mylogs.txt", ios::out | ios::app);
            myfile << "Navigation Started!  ||  ";
            myfile.close();
    ...
    bool LocalPlanner::computeVelocityCommands(geometry_msgs::Twist &cmd_vel)
    {
        ...
        linearVel = ((coefVel * ((0.4 * log((3.5 * (dmin_temp - 0.15)) + 0.0)) / (exp(0.883)) + (exp(1.57) / 6.5))) + 0.01)*realsysfactor;
        ...
        // angularVel = (0.2 * phiFinal/2.2 * 0.7 * ((exp(-4 * 0.1) / 2.0) + 1))*realsysfactor; //BUNU KULLANIYORMUŞUM BU NEY Kİ Bİ DURSUN 24 KAS 2023
    ...
    double LocalPlanner::LLCallback()
    {
        ...
        else
        {
            ...
            {   // Bu for döngüsü içinde hem açısal olarak midpoint hesabı yapılır hem de hafıza kısmından gelen koordinat oluşturma ve onların midpoint hesabı yapılır.
                ...
                memory_array[i][0] = lidar_coord_x - d1_temp*cos(M_PI*(robot_pose_theta_manipulated + (alpha_temp+12.0))/180.0); //d1 den gelen X koord
                memory_array[i][1] = lidar_coord_y + d1_temp*sin(M_PI*(robot_pose_theta_manipulated + (alpha_temp+12.0))/180.0); //d1 den gelen y koord
                memory_array[i][2] = lidar_coord_x - d2_temp*cos(M_PI*(robot_pose_theta_manipulated + (beta_temp+12.0))/180.0);  //d2 den gelen X
                memory_array[i][3] = lidar_coord_y + d2_temp*sin(M_PI*(robot_pose_theta_manipulated + (beta_temp+12.0))/180.0);  //d2 den gelen Y
            ...
            for (int i=0; i<rows ;i++) //bu döngünün içince her gap midpointe ait x ve y koordinatları midpoint vektörüne pushlanır. midpoint vektörü hafıza vektörüne pushlanır. bir cycleda 2 gap görüldüyse yine teker teker pushlanır.
            {
                ...
                if(midpoint_coords[i][2] > 0.7) //genişliği 0.45'ten küçük olan gapler hafızaya atılmaz
                {
        ...
        for (int i=0; i < gaps_in_memory.size(); i++)
        {
            if (gaps_in_memory[i][2] < 0.55) //0,45 ten kucuk olan gapler odullendirilmez.
            {
                gaps_in_memory[i][2] = 0.1;
            }
            else if (diff_to_goal_new[i] <= 30)
            {
                gaps_in_memory[i][2] = gaps_in_memory[i][2] * 12;
            ...
            {
                gaps_in_memory[i][2] = gaps_in_memory[i][2] * 4.5;
            }
