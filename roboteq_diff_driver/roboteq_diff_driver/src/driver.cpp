  ...
  gear_ratio(0)
{
  ...
  nhLocal.param("wheel_circumference", wheel_circumference, 0.324);
  ROS_INFO_STREAM("wheel_circumference: " << wheel_circumference);
  nhLocal.param("track_width", track_width, 0.575);
  ROS_INFO_STREAM("track_width: " << track_width);
  nhLocal.param("encoder_ppr", encoder_ppr, 360);
  ROS_INFO_STREAM("encoder_ppr: " << encoder_ppr);
  nhLocal.param("encoder_cpr", encoder_cpr, 1440);
  ROS_INFO_STREAM("encoder_cpr: " << encoder_cpr);
  nhLocal.param("max_amps", max_amps, 30.0);
  ROS_INFO_STREAM("max_amps: " << max_amps);
  nhLocal.param("max_rpm", max_rpm, 4000);
  ROS_INFO_STREAM("max_rpm: " << max_rpm);
  nhLocal.param("gear_ratio", gear_ratio, 32);
...
void MainNode::cmdvel_callback( const geometry_msgs::Twist& twist_msg)
{
  ...
  // wheel speed (m/s)
  float right_speed = twist_msg.linear.x*gear_ratio/3.2 - track_width * (twist_msg.angular.z * 2.2) / .2;
  float left_speed = twist_msg.linear.x*gear_ratio/3.2 + track_width * (twist_msg.angular.z * 2.2) / .2;
#ifdef _CMDVEL_DEBUG
ROS_DEBUG_STREAM("cmdvel speed right: " << right_speed << " left: " << left_speed);
  ...
  if (open_loop)
  {
    ...
    int32_t right_power = right_speed / wheel_circumference * 60.0 / max_rpm * 4000.0;
    int32_t left_power = left_speed / wheel_circumference * 60.0 / max_rpm * 4000.0;
#ifdef _CMDVEL_DEBUG
...
void MainNode::cmdvel_setup()
{
  ...
  controller.write("^KP 1 11\r");
  controller.write("^KP 2 11\r");
  controller.write("^KI 1 3\r");
  controller.write("^KI 2 3\r");
