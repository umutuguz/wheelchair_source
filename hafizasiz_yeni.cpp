...
namespace local_planner
{
    ...
    bool LocalPlanner::computeVelocityCommands(geometry_msgs::Twist &cmd_vel)
    {
        ...
        linearVel = ((coefVel * ((0.4 * log((3.5 * (dmin_temp - 0.15)) + 0.0)) / (exp(0.883 * phiFinal_abs)) + (exp(1.57 - phiFinal_abs) / 6.5))) + 0.01)*realsysfactor;
    ...
    double LocalPlanner::LLCallback()
    {
        ...
        else
        {
            ...
            {   // Bu for döngüsü içinde hem açısal olarak midpoint hesabı yapılır hem de hafıza kısmından gelen koordinat oluşturma ve onların midpoint hesabı yapılır.
                ...
                memory_array[i][0] = lidar_coord_x - d1_temp*cos(M_PI*(robot_pose_theta_manipulated + (alpha_temp+12))/180.0); //d1 den gelen X koord
                memory_array[i][1] = lidar_coord_y + d1_temp*sin(M_PI*(robot_pose_theta_manipulated + (alpha_temp+12))/180.0); //d1 den gelen y koord
                memory_array[i][2] = lidar_coord_x - d2_temp*cos(M_PI*(robot_pose_theta_manipulated + (beta_temp+12))/180.0);  //d2 den gelen X
                memory_array[i][3] = lidar_coord_y + d2_temp*sin(M_PI*(robot_pose_theta_manipulated + (beta_temp+12))/180.0);  //d2 den gelen Y
        ...
        for (int i=0; i < gaps_in_memory.size(); i++)
        {
            if (gaps_in_memory[i][2] < 0.85) //0,45 ten kucuk olan gapler odullendirilmez.
            {
                gaps_in_memory[i][2] = 0.1;
            }
            else if (diff_to_goal_new[i] <= 30)
            {
                gaps_in_memory[i][2] = gaps_in_memory[i][2] * 4.2;
            ...
            {
                gaps_in_memory[i][2] = gaps_in_memory[i][2] * 2.1;
            }
            else if (diff_to_goal_new[i] <= 90 && diff_to_goal_new[i] > 60)
            {
                gaps_in_memory[i][2] = gaps_in_memory[i][2] * 1.4;
            }
